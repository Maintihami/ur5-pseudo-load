            ...
            try:
                ...
                for force in forces:
                    ...
                    if force in force_indices:
                        ...
                        invalid_forces.append(force)

                if invalid_forces:
                    for invalid_force in invalid_forces:
                        print(f"Invalid force: {invalid_force}")
                    print("Please enter valid forces from the list:", list(force_indices.keys()))
                else:
                    break
            except ValueError:
...
Mx, My, Mz = [], [], []
tcp_pose_list = []
dx, dy, dz, d_rx, d_ry, d_rz = 3.8, 3.8, 15.5, 0, 0, 0
"""-----------------------------main loop-----------------------------------"""
try:
    ...
    while True:
        try:
            ...
            offset_AB = np.array([dx, dy, dz, d_rx, d_ry, d_rz])  # Replace with actual offset values
            # Calculate the TCP position at B
            tcp_position_B = tcp + offset_AB
            tcp_pose_list.append(tcp)
            # tcp=np.array(tcp)
            # speed=np.array(state.actual_TCP_speed)
            # Interest_point=tcp+
...
with open(output_position_path, 'a') as p:
    for pos in tcp_pose_list:
        A = convert_coordinates(pos)        # Convert the coordinates to point A
        p.write(f"O = {pos[0]:<10.5f}\t{pos[1]:<10.5f}\t{pos[2]:<10.5f}\t{pos[3]:<10.5f}\t{pos[4]:<10.5f}\t{pos[5]:<10.5f}\n")
        p.write(f"A = {A[0]:<10.5f}\t{A[1]:<10.5f}\t{A[2]:<10.5f}\t{A[3]:<10.5f}\t{A[4]:<10.5f}\t{A[5]:<10.5f}\n")
